...
class PeopleController(private val jdbc:JdbcTemplate){
 @GetMapping("/researchers") fun researchers(@RequestParam(required=false) q:String?):List<Map<String,Any?>> { val t="%${q?:""}%";return jdbc.queryForList("select rp.id,rp.user_id researcher_user_id,u.display_name,u.email,u.job_title,ou.name unit,rp.orcid,rp.expertise,rp.career_stage,rp.active from researcher_profiles rp join users u on u.id=rp.user_id left join organisation_units ou on ou.id=u.organisation_unit_id where lower(u.display_name) like lower(?) or lower(u.email) like lower(?) order by u.display_name limit 300",t,t) }
