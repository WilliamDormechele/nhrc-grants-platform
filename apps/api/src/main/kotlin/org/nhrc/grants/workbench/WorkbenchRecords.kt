...
class WorkbenchRecords(private val store: WorkbenchStore, private val auth: WorkbenchAuth) {
    ...
    /** Resolve labels in bounded batches, rather than issuing one lookup per table cell. */
    private fun labels(r: GrantResource, rows: List<GrantRow>, actor: GrantActor): List<GrantRow> {
        val references=mutableMapOf<String,Map<String,String>>()
        for(f in r.fields.filter { it.kind=="lookup" }) {
            val ids=rows.mapNotNull { store.uuid(it,f.name) }.distinct()
            if(ids.isEmpty()) continue
            val target=when(f.lookup) {
                "users" -> "users" to "display_name"
                "awards" -> "awards" to "title"
                else -> {
                    val linked=resource(f.lookup!!)
                    if(!actor.hasAny(linked.readers)) continue
                    linked.table to (linked.fields.firstOrNull { it.name in setOf("name","legal_name","title","subject","reference","item_description") }?.name ?: "id")
                }
            }
            references[f.name]=store.rows("select id,${target.second}::text label from ${target.first} where id in (${ids.joinToString { "?" }})",*ids.toTypedArray()).associate { it["id"].toString() to it["label"].toString() }
        }
        return rows.map { row ->
            val names=references.mapNotNull { (field,choices)->choices[row[field]?.toString()]?.let { field to it } }.toMap()
            row+mapOf("_references" to names)+(if(r.key=="researchers") mapOf("name" to names["user_id"]) else emptyMap())
        }
    }
    fun list(key: String, actor: GrantActor, query: String, page: Int, size: Int): GrantRow {
        ...
        val (condition,params)=scope(r,actor)
        val expressions=r.fields.filter { it.kind in setOf("text","textarea","email") }.take(6).map { "coalesce(t.${it.name},'') ilike ?" }.toMutableList()
        if(key=="researchers") expressions.add("exists(select 1 from users u where u.id=t.user_id and u.display_name ilike ?)")
        val search=if(expressions.isEmpty()) "true" else expressions.joinToString(" or ","(",")")
        val args=params+expressions.map { "%$query%" }
        val where="$condition and $search"
        ...
        return mapOf("items" to labels(r,data,actor).map { decorate(r,it,actor) },"total" to count,"page" to page,"size" to size,"hasMore" to ((page.toLong()+1)*size<count))
    }
    fun get(key: String,id: UUID,actor: GrantActor): GrantRow {
        ...
        return decorate(r,labels(r,listOf(row),actor).single(),actor)
    }
    ...
    fun canEdit(r: GrantResource,row: GrantRow,actor: GrantActor): Boolean {
        ...
        return status in setOf("DISCOVERED","DRAFT","RETURNED","OPEN","PLANNED","ACTIVE","SCHEDULED","NOT_STARTED","IN_PROGRESS","OUTSTANDING","CLAIMED","RECORDED","PENDING","NEW")
    }
    ...
    private fun consistency(r: GrantResource,values: Map<String,Any?>,before: GrantRow?) {
        ...
        if(r.key=="purchase-orders") {
            val requisitionId=UUID.fromString(values["requisition_id"].toString())
            val req=store.one("procurement_requisitions",requisitionId,true)
            ...
            val others=store.rows("select coalesce(sum(amount),0) amount from purchase_orders where requisition_id=? and status<>'CANCELLED' and id<>?",requisitionId,before?.let { store.uuid(it,"id") }?:UUID(0,0)).single()
            require((values["amount"] as BigDecimal)+store.decimal(others,"amount")<=store.decimal(req,"amount")) { "Combined orders exceed the approved requisition" }
            GrantDiligenceRules.requireCurrentApproval(store.one("suppliers",UUID.fromString(values["supplier_id"].toString()),true))
        }
    ...
    fun save(key: String,id: UUID?,input: GrantRecordInput,actor: GrantActor): GrantRow {
        ...
        return decorate(r,labels(r,listOf(after),actor).single(),actor)
    }
    @Transactional
    fun action(key: String,id: UUID,input: GrantActionInput,actor: GrantActor): GrantRow {
        ...
        require(input.values.isEmpty()) { "This record decision does not accept additional editable fields" }
        ...
        if(key=="award-partners")GrantDiligenceRules.requireCurrentApproval(store.one("partners",store.uuid(before,"partner_id")!!,true))
        if(key=="purchase-orders"&&input.action=="ISSUE")GrantDiligenceRules.requireCurrentApproval(store.one("suppliers",store.uuid(before,"supplier_id")!!,true))
        when(key) {
        ...
        val result=decorate(r,labels(r,listOf(after),actor).single(),actor)
