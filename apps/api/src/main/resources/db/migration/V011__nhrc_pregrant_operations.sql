CREATE TABLE pregrant_expressions_of_interest (
...
CREATE INDEX idx_application_requirements_app_status ON application_requirements(application_id,status,due_at);

CREATE TABLE funder_portal_records (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_id UUID NOT NULL REFERENCES applications(id) ON DELETE CASCADE,
...
CREATE TABLE application_quality_checks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_id UUID NOT NULL REFERENCES applications(id) ON DELETE CASCADE,
...
CREATE TABLE funder_communications (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    application_id UUID NOT NULL REFERENCES applications(id) ON DELETE CASCADE,
...
CREATE INDEX idx_funder_communications_app_date ON funder_communications(application_id,communication_at DESC);

CREATE TABLE award_handovers (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
FROM applications a
CROSS JOIN (VALUES
 ('TECHNICAL','Technical proposal / concept note'),
 ('WORKPLAN','Work plan'),
 ('BUDGET','Budget and justification'),
 ('CV','Required CVs / biosketches'),
 ('LETTER','Letters of support / commitment'),
 ('INSTITUTIONAL','Required institutional documents'),
 ('APPROVAL','Required internal approvals'),
 ('CERTIFICATION','Required certifications'),
 ('PORTAL','Funder portal requirements'),
 ('SUBMISSION','Submission readiness and proof')
