...
function ApplicationHub({mode,app,applications,workspace,select,act,run,users,researchers}:{mode:string;app:Row|null;applications:Row[];workspace:any;select:(id:string)=>void;act:(a:Action)=>void;run:(path:string,method:string,body?:any)=>Promise<void>;users:Row[];researchers:Row[]}){
 ...
 return <><AppSelector applications={applications} value={app.id} onChange={select}/><div className="appSummary"><div><span className="eyebrow">{app.reference}</span><h2>{app.title}</h2><p className="subtle">{app.lead_researcher||"Lead researcher not assigned"} · deadline {date(app.deadline_at)}</p></div><div className="actions">{badge(app.stage)}{app.stage==="DIRECTOR_DECISION"?<button className="btn" onClick={()=>act({kind:"assign",row:app})}>Assign researcher</button>:app.stage==="ASSIGNED"&&assignments.find(a=>a.response==="PENDING")?<button className="btn" onClick={()=>act({kind:"assignmentResponse",row:{...app,assignmentId:assignments.find(a=>a.response==="PENDING")?.id}})}>Researcher response</button>:next&&!["SUBMITTED","OUTCOME_RECORDED","AWARDED","CLOSED"].includes(app.stage)&&<button className="btn" onClick={()=>act({kind:"advance",row:app})}>Advance workflow</button>}</div></div>
...
function ActionSheet({action,users,researchers,close,done,busy}:{action:NonNullable<Action>;users:Row[];researchers:Row[];close:()=>void;done:(path:string,method:string,body:any)=>Promise<void>;busy:boolean}){
 ...
 async function submit(e:any){e.preventDefault();let path="",method="POST",body:any={};switch(action.kind){
  ...
  case "fit":path="/api/opportunities/"+row.id+"/fit";method="PATCH";body={score:Number(form.score||0),rationale:form.rationale||null};break;
  ...
  case "assign":path="/api/applications/"+row.id+"/assignments";body={researcherId:form.researcherId,assignedBy:form.assignedBy||null};break;
  case "assignmentResponse":path="/api/applications/assignments/"+row.assignmentId+"/response";body={response:form.response||"ACCEPTED",note:form.note||null};break;
 ...
 {action.kind==="fit"&&<>{input("Institutional fit score (%)","score","number",true)}{textarea("Fit rationale","rationale")}</>}
 ...
 {action.kind==="assign"&&<>{researcherSelect("Researcher","researcherId")}{userSelect("Assigned by","assignedBy")}</>}
 {action.kind==="assignmentResponse"&&<><label>Response<select value={form.response||"ACCEPTED"} onChange={e=>set("response",e.target.value)}><option>ACCEPTED</option><option>DECLINED</option></select></label>{textarea("Response note","note")}</>}
