...
"NHRC Institutional Profile":"Institutional mandate, strategic themes, capabilities and context used by the transparent fit model.",
...
function Body({name,group,data,rows,select,go}:{name:string;group:string;data:any;rows:Row[];select:(r:Row)=>void;go:(g:string,i:string)=>void}){
 ...
 if(name==="NHRC Institutional Profile")return <InstitutionalProfileView data={data||{}}/>;
...
function InstitutionalProfileView({data}:{data:Row}){const list=(v:any)=>Array.isArray(v)?v:[];return <><div className="stats"><Tile l="Strategic themes" v={list(data.strategicThemes).length} n="Institutional priorities"/><Tile l="Capabilities" v={list(data.capabilities).length} n="Delivery capacity"/><Tile l="Methods & platforms" v={list(data.methodsPlatforms).length} n="Research infrastructure"/><Tile l="Contexts" v={list(data.populationsContexts).length+list(data.geographyKeywords).length} n="Population and geography"/></div><div className="grid"><Panel title="Institutional identity"><div className="checks"><p><strong>{data.name||"Navrongo Health Research Centre"}</strong>{data.organisationType||"Research centre"} · {data.countryCode||"GHA"}</p><p><strong>Mission</strong>{data.mission||"—"}</p></div></Panel><Panel title="Fit model"><div className="notice">Institutional fit is calculated from this governed profile. It is separate from funder eligibility and is not a probability of winning.</div><Checks items={["Thematic alignment: 40%","Institutional capabilities/platforms: 25%","Researcher expertise capacity: 20%","Population/geographic context: 15%"]}/></Panel></div><div className="grid"><Panel title="Strategic themes"><div className="plainTags">{list(data.strategicThemes).map((x:string)=><span key={x}>{x}</span>)}</div></Panel><Panel title="Capabilities and platforms"><div className="plainTags">{[...list(data.capabilities),...list(data.methodsPlatforms)].map((x:string)=><span key={x}>{x}</span>)}</div></Panel></div></>}
