...
export default function ConfigurationWorkspace({kind}:{kind:"forms"|"templates"}){
  ...
  const [selected,setSelected]=useState<Row|null>(null),[editing,setEditing]=useState(false),[name,setName]=useState(""),[templateDescription,setTemplateDescription]=useState(""),[json,setJson]=useState(""),[error,setError]=useState("");
  ...
  const open=(item:Row)=>{setSelected(item);setEditing(false);setError("");setName(item.name||"");setTemplateDescription(item.description||"");setJson(JSON.stringify(kind==="forms"?item.schema_json:item.content_json,null,2));};
  const save=async()=>{
    if(!selected)return;setError("");
    let parsed:any;try{parsed=JSON.parse(json);}catch{setError("Enter valid JSON.");return;}
    try{
      await client("/configuration/"+kind+"/"+selected.id,{method:"PATCH",body:JSON.stringify({values:kind==="forms"?{name,schema_json:parsed}:{name,description:templateDescription,content_json:parsed}})});
  ...
  const pageDescription=kind==="forms"?"Versioned institutional forms used across the grant lifecycle.":"Versioned NHRC templates for proposals, reviews, finance, procurement, laboratory, governance and closeout.";
  return <>
    <Heading title={title} description={pageDescription}><button className="btn" onClick={load.reload} disabled={load.loading}>Refresh</button></Heading>
      ...
      <div className="panelbody">
        ...
        {editing?<>
          <div className="formgrid">
            ...
            {kind==="templates"&&<div className="field full"><label>Description</label><textarea rows={3} value={templateDescription} onChange={e=>setTemplateDescription(e.target.value)}/></div>}
